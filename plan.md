...
- Ponencias (`content/ponencias/` + `layouts/ponencias/{list,single}.html`): **renombrada y rediseñada** — ya no reutiliza `data/cv.yaml → publications` (campo eliminado). Cada ponencia es una página independiente (`title`, `date`, `place`, `link` opcional, `tags`); el listado las agrupa cronológicamente por mes/año (`.Pages.GroupByDate ":January 2006"`, con truco del prefijo `:` para nombres de mes en español — hay que recortar el `:` manualmente con `strings.TrimPrefix`, Hugo no lo hace solo). Si la ponencia tiene `link`, el título enlaza directo al recurso externo; si no, enlaza a su propia ficha. La sección "Publicaciones y Conferencias" del `/cv/` se sustituyó por un enlace a `/ponencias/`. **Rediseño de escalabilidad (feedback UX)**: la primera versión (barra plana de píldoras año+mes) dejó de funcionar en cuanto hubo contenido real — con 9 años y ~20 meses distintos se convertía en una pared de información ilegible e imposible de escanear. Se sustituyó por:
  - **Acordeón nativo por año** (`<details>`/`<summary>`, sin JS): solo el año más reciente abre por defecto (`{{ if eq $i 0 }} open{{ end }}` sobre `.Pages.GroupByDate "2006"`), el resto queda colapsado en una sola línea con contador — escala a cualquier número de años sin amontonarse. Dentro de cada año, los meses se generan igual que antes (`GroupByDate ":January 2006"` anidado). Deep-linking (`#year-2026`, `#2026-02`) se conserva; los navegadores abren automáticamente el `<details>` al enlazar a un ancla interior.
  - **Filtro real por tipo** (Todas / Ponencias / Prensa, con contador): resuelve el problema de fondo ("difícil de filtrar") en vez de solo ayudar a saltar de fecha. Implementado con radios accesibles + `assets/js/site.js` (oculta `.ponencia-item` por `data-tags`, y en cascada oculta `.ponencias-group`/`.ponencias-year` que se quedan sin ítems visibles).
- **Contenido real cargado** (40 entradas, 2017–2025): el usuario facilitó un listado en Markdown de ponencias y apariciones en prensa, desordenado; se generó con un script Python (`/private/tmp/.../scratchpad/gen_ponencias.py`, no forma parte del repo) para crear las páginas con front matter consistente. Se etiquetó cada entrada como `Ponencia` (cuando fue él quien habló/presentó) y/o `Prensa` (cobertura mediática, entrevistas, menciones). Entradas con varias fuentes sobre el mismo evento (RootedCON Valencia 2024, nombramiento como CTO de Ontinet.com, Hacker vs CIO, "Los ISP refuerzan su ciberdefensa...") se agruparon en una sola página con `links: [{label, url}]` en vez de duplicar entradas. Se añadió soporte de `links` (referencias múltiples) a `layouts/ponencias/single.html`, reutilizando el mismo bloque "Referencias" que Proyectos.
  - **Pendiente del usuario**: 11 ítems del listado original no tenían fecha y se dejaron fuera a propósito (no se inventó ninguna fecha) — CyLEconomica, IBERRADIO "De las válvulas a los bits", Webcast EMCOM España, Hack & Beers ×2 (marítimas, EVA), PaellaCON, podcast Tardeo con G&G, HackerCar (Volvo XC90), Grupo Sothis, IMF Smart Education, CyberLideria Magazine. En cuanto Alejandro aporte al menos mes/año se añaden igual que el resto. La entrada "IBERRADIO 2019" solo tenía año (sin mes) — se cargó con fecha provisional de junio 2019, pendiente de confirmar el mes real.
